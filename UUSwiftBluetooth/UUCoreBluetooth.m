...
- (void) uuDisconnectPeripheral:(nonnull CBPeripheral*)peripheral timeout:(NSTimeInterval)timeout;
{
    ...
    if (![self uuIsPoweredOn])
    {
        ...
        [self uuNotifyDisconnect:peripheral error:nil];
        return;
    }
    ...
    [peripheral startTimer:timerId timeout:timeout block:^(CBPeripheral * _Nonnull peripheral)
    {
        ...
        [UUCoreBluetooth cancelWatchdogTimer:timerId];
        [self uuNotifyDisconnect:peripheral error:[NSError uuCoreBluetoothError:UUCoreBluetoothErrorCodeTimeout]];
...
- (void) uuNotifyDisconnect:(nonnull CBPeripheral*)peripheral error:(nullable NSError*)error
{
    UUCentralManagerDelegate* delegate = [self uuCentralManagerDelegate];
...
@property (assign, readwrite) BOOL isConfiguredForStateRestoration;
...
static UUCoreBluetooth* theSharedInstance = nil;
...
+ (nonnull instancetype) sharedInstance
{
    if (theSharedInstance == nil)
    {
        ...
        theSharedInstance = [[UUCoreBluetooth alloc] initWithCentralOptions:opts];
    }
...
+ (void) setSharedInstanceInitOptions:(nullable NSDictionary<NSString*, id>*)options
{
    ...
    if (theSharedInstance != nil)
    {
        NSString* existingRestoreId = [theSharedInstance.options uuSafeGetString:CBCentralManagerOptionRestoreIdentifierKey];
        NSString* incomingRestoreId = [theSharedInstanceInitOptions uuSafeGetString:CBCentralManagerOptionRestoreIdentifierKey];
        if (![existingRestoreId isEqualToString:incomingRestoreId])
        {
            UULog(@"UUCoreBluetooth init options have changed! Setting theSharedInstance to nil");
...
- (id) initWithCentralOptions:(nullable NSDictionary<NSString*, id>*)options
{
    ...
    if (self)
    {
        ...
        if (options && [options valueForKey:CBCentralManagerOptionRestoreIdentifierKey])
        {
            ...
            self.isConfiguredForStateRestoration = YES;
        }
        else
        {
            self.delegate = [[UUCentralManagerDelegate alloc] init];
            self.isConfiguredForStateRestoration = NO;
        }
...
- (void) handleCentralStateChanged:(CBManagerState)state
{
    for (UUPeripheral* p in self.peripherals.allValues)
    {
        ...
        {
            [self.centralManager uuNotifyDisconnect:p.peripheral error:nil];
        }
    }
